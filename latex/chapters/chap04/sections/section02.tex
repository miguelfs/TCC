...
A partir das equações \ref{chap_04:base} e \ref{chap_04:error}, que descrevem o
...
Em aplicações de \textit{Speech Enhancement}, uma das implementações consiste em
...
% \frac{S_{yx}[k]}{S_{xx}[k]}
% No domínio da STFT, obtém-se os ganhos do filtro ao minimizar a equação
% equivalente à \eqref{chap_4:eq_error} \cite{parchami2016}:
% \begin{equation}\label{chap04:w}
%     \hat{W}[k, l] = \operatorname*{argmin}_W \mathbb{E}[\,|D[k,l] - H[k,l]X[k,l|^2\,]
% \end{equation}
% A solução para a equação \ref{chap04:w} é descrita por:
% \begin{equation}
%     \hat{W}[k,l] = \frac{S_{yx}[k,l]}{S_{xx}[k, l]}
% \end{equation}
% Finalmente, o espectro tratado de fala é obtido ao aplicar o filtro no sinal
% original:
% \begin{equation}
%     \hat{X}[k, l] = \hat{W}[k,l]X[k,l]
% \end{equation}
